...
// 删除过期的CVE编号和产品关联关系
pub fn del_expire_product(conn: &mut PgConnection, id: &str, product_set: HashSet<Vec<u8>>) {
...
// 更新产品描述和元数据链接信息
pub fn update_products(conn: &mut PgConnection, args: UpdateProduct) -> DBResult<Product> {
...
pub fn with_archive_cpe(path: PathBuf) {
  ...
  for cpe_item in c.cpe_item.into_iter() {
    ...
    if current == Some(product.clone()) {
      ...
      let description = get_title(all_titles).unwrap_or_default();

use diesel::{
    BelongingToDsl, ExpressionMethods, Insertable, PgConnection, QueryDsl, RunQueryDsl, SelectableHelper, TextExpressionMethods,
};
...
impl Product {
  // 创建产品
  pub fn create(conn: &mut PgConnection, args: &CreateProduct) -> DBResult<Self> {
  ...
  pub fn update(conn: &mut PgConnection, args: &UpdateProduct) -> DBResult<Self> {
  ...
  // 查询产品从提供商的id
  pub fn query_by_id(conn: &mut PgConnection, args: &QueryProductById) -> DBResult<Self> {
    Ok(

...
use diesel::{BoxableExpression, ExpressionMethods, PgConnection, QueryDsl, RunQueryDsl};
...
impl Cve {
  // 创建CVE
  pub fn create(conn: &mut PgConnection, args: &CreateCve) -> DBResult<Self> {
  ...
  pub fn update_translated(conn: &mut PgConnection, id: &str, description: &str) {
  ...
  pub fn create_or_update(conn: &mut PgConnection, args: &CreateCve) -> DBResult<Self> {
    if let Err(err) = diesel::insert_into(cves::table).values(args).execute(conn) {
      // 重复了，说明已经存在CVE
      return match err {
        DieselError::DatabaseError(DatabaseErrorKind::UniqueViolation, _) => {
  ...
  // 查单个cve不联cvss表
  pub fn query_by_id(conn: &mut PgConnection, id: &str) -> DBResult<Self> {

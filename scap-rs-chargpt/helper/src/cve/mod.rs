...
pub(crate) async fn async_cve(param: CveParameters) {
  ...
  let resp = match api.cve(param).await {
    Ok(r) => r,
    Err(e) => {
      eprintln!("NVD API parse error: {:?}", e);
      return;
    }
  };
  if let Object::Vulnerabilities(vs) = resp.results {
    for v in vs {
      ...
      if let Err(e) = import_from_api(connection_pool.get().unwrap().deref_mut(), v.cve) {
        eprintln!("Kayıt eklenirken hata oluştu: {:?}", e);
      }
    }
  }
}

fn import_from_archive(connection: &mut PgConnection, cve_item: CVEItem) -> DBResult<String> {

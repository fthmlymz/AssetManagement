...
impl KnowledgeBase {
  // 创建漏洞利用
  pub fn create(conn: &mut PgConnection, args: &CreateKnowledgeBase) -> DBResult<Self> {
  ...
  pub fn delete(conn: &mut PgConnection, name: &str, source: KBSource) -> DBResult<usize> {
  ...
  pub fn query_by_cve(conn: &mut PgConnection, id: &str) -> DBResult<Vec<Self>> {
